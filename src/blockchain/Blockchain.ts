...
class Blockchain {
    ...
    return [
      {
        ...
        content: this.unconfirmedTransactions.length
      },
  ...
  getRecentTransactions(quantity) {
    ...
    for (let i = this.blocks.length - 1; i >= 0 && recentTransactions.length < quantity; i--) {
      const transactions = this.blocks[i].transactions.reverse();
   ...
   */
  mineTheBlock() {
    const block = this.getNextBlock(this.unconfirmedTransactions);
    this.blocks.push(block);
    this.unconfirmedTransactions = [];
    this.luckyNumber = this._generateLuckyNumber();
  }
  ...
  _tryToMineTheBlock(
    ...
    if (luckyNumber === this.luckyNumber) {
      const transaction = new Transaction('No input (newly generated coins)', rewardAddress, amount);
